 ...
 * USA
 */
package org.languagetool.dictionarybuilder;
...
public class DictionaryBuilder {
	...
	// Valid for tagger dictionaries (wordform_TAB_lemma_TAB_postag) or spelling
	// dictionaries (wordform)
	...
	protected File buildFSA(File inputFile) throws Exception {
		...
		String[] buildOptions = { "--exit", "false", "-i", inputFile.toString(), "-o", resultFile.toString(), "-f",
				serializationFormat.toString() };
		...
		try (BufferedWriter bw = new BufferedWriter(
			...
			while ((line = br.readLine()) != null) {
				...
				if (m.matches()) {
					...
					String freqChar = Character
							.toString((char) (FIRST_RANGE_CODE + normalizedFreq * FREQ_RANGES_OUT / FREQ_RANGES_IN));
		...
		try (Scanner scanner = new Scanner(inputFile, encoding);
			...
			while (scanner.hasNextLine()) {
				...
				if (parts.length == 3) {
					...
					System.err.println("Invalid input, expected three tab-separated columns in " + inputFile + ": "
							+ line + " => ignoring");
				}
			}
			scanner.close();
